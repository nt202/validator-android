...
public class ObjectSchema extends Schema {
    ...
    public Map<Pattern, Schema> getPatternProperties() {
        return StreamSupport.stream(patternProperties.entrySet())
                .map(entry -> new AbstractMap.SimpleEntry<>(Pattern.compile(entry.getKey().toString()), entry.getValue()))
                .collect(toMap(
                        (Map.Entry<Pattern, Schema> entry) -> entry.getKey(),
                        (Map.Entry<Pattern, Schema> entry) -> entry.getValue()
                ));
    }
    ...
    private void describePropertyDependenciesTo(JSONPrinter writer) {
        writer.key("dependencies");
        writer.object();
        Stream.of(propertyDependencies).forEach(entry -> {
            writer.key(entry.getKey());
            writer.array();
            Stream.of(entry.getValue()).forEach(writer::value);
            writer.endArray();
        });
        writer.endObject();
    }
